...
from uniform_motif_sampling import uniform_motif_accept_reject, uniform_motifs_accept_reject, uniform_motifs_with_ic_rw_harmonic
...
def main_experiment_final(trials=50):
    """compare ME, TU(RS) and TU(RW) runtimes"""
    num_motifs = 3
    me_times = []
    ar_times = []
    rw_times = []
    for trial in trange(trials):
        ...
        L = random.randrange(5,20)
        ic = (random.random()*1.9) * L
        print "starting on:",n, L, ic
        print "ME"
        t = time.time()
        me_motifs = maxent_motifs_with_ic(n,L,ic,num_motifs)
        me_time = time.time() - t
        print "AR"
        t = time.time()
        ar_motifs = uniform_motifs_accept_reject(n,L,ic,num_motifs)
        ar_time = time.time() - t
        print "RW"
        t = time.time()
        rw_motifs = uniform_motifs_with_ic_rw_harmonic(n, L, ic, num_motifs)
        rw_time = time.time() - t
        me_times.append(me_time)
        ar_times.append(ar_time)
        rw_times.append(rw_time)
    return (me_times, ar_times, rw_times)
